...
int al_clear(ArrayList* pList)
{
    ...
    if (pList!=NULL)
    {
        ...
        (void**)realloc(pList->pElements, (sizeof(void *) * AL_INITIAL_VALUE ));
...
int al_sort(ArrayList* pList, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(pList!=NULL && pFunc!=NULL && (order==1 || order==0) && al_len(pList)>0)
    {
        for(i=0; i<(al_len(pList))-1; i++)
        {
            for(j=i+1; j<(al_len(pList)); j++)
            {
                ...
                {
                    plistAuxiliar=al_get(pList,j);

                    al_set(pList,j,al_get(pList,i));

                    al_set(pList,i,plistAuxiliar);

                }
                if(pFunc(al_get(pList,i),al_get(pList,j))==1 && order==1)
                {
                    plistAuxiliar=pList->get(pList,j);

                    pList->set(pList,j,pList->get(pList,i));

                    pList->set(pList,i,plistAuxiliar);
